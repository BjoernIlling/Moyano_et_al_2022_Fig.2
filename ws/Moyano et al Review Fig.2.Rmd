...
data = read.csv2('../data/final_dataset_yury_csv_2020_05_21.csv',sep=",")
str(data)
head(data)
...
data = data %>% mutate(Year=as.numeric(Column1),
                       ...
                       #GERAS_Age1_adj_thousands=as.numeric(GERAS_Age1_adj/1000))
                       
```

Read in additional data from https://standardgraphs.ices.dk/ViewCharts.aspx?key=13313


```{r prepare2, results='markdown', eval=TRUE, hidden=TRUE}

data2 = read.csv2('../data/ICES_WBSS_herring_data_2021_05_21.csv',sep=",")
str(data2)


data2 = data2 %>% mutate(Year=as.numeric(Year),
...
```{r}
scaleFactor <- max(data2$Recruitment_billions)/max(data$N20.Age.0_billions)
```

```{r}
recruitment_plot2<-ggplot()+
  geom_bar(data=data2,aes(y=Recruitment_billions ,x=Year),stat="identity", fill="skyblue", alpha=0.5, width = 0.7)+
  geom_errorbar(data=data2,aes(x=Year, ymin=Low.Recruitment_billions, ymax=High.Recruitment_billions), width=0.4, colour="skyblue", alpha=1, size=0.7)+
...
recruitment_plot2
```
Previous orange colour used: #FFAE42
